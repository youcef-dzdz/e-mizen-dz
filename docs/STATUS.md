...
## Dernière Session
Date: 2026-06-29 (Session 012 — clôture)
Ce qui a été fait: UI inscription avocat COMPLÈTE construite, buildée (0 erreur) et testée en réel (rappel — voir Journal Session 012 pour le détail 3a/3b/3c). PUIS : écrit le trigger 012 promote_avocat_on_email_confirm (AFTER UPDATE OF email_confirmed_at sur auth.users, défensif : ne casse jamais la confirmation, appelle register_avocat + supprime pending) + son test comportemental simulé + script reset_trigger_test_account.sql (filet de récupération) — commit 501beab, ⚠️ NON EXÉCUTÉ EN BASE. CODEMAP rattrapé (migrations 006→012 + composants avocat, dette Rule 23 depuis Session 005) — commit d31f8c2. Audit complet du projet réalisé.
Prochaine tâche: (1) EXÉCUTER migration 012 sur Supabase (Ctrl+A → Run) puis lancer le test promote_avocat_on_confirm.test.sql (compte test jetable 22349d59 confirmé ; reset_trigger_test_account.sql prêt en filet) ; (2) diagnostiquer auth_error=1 (identifié : liens de confirmation expirés — otp_expired dans les logs — car ouverts trop tard ; + rate limit email Supabase 2/h ; solution racine = Resend SMTP) ; (3) configurer Resend SMTP.
Résumé point (reprise): Flux inscription avocat COMPLET côté code (004+011 exécutés et validés en réel — pending écrit confirmé en base ; 009+010 exécutés ; 012 ÉCRIT+COMMITÉ mais NON EXÉCUTÉ). UI complète et testée en réel. Il RESTE à exécuter 012 en base + le tester (1re tâche next session, à tête reposée — 012 touche auth.users). Puis fix auth_error=1 (liens expirés / SMTP) via Resend.
...
**Trigger 012 promotion (Session 012 suite) ÉCRIT + COMMITÉ (501beab) mais NON EXÉCUTÉ en base** — 1re tâche de la prochaine session (exécution + test, compte jetable 22349d59 + reset_trigger_test_account.sql prêt en filet). CODEMAP rattrapé (d31f8c2).
...
- [ ] **[Phase 1 — À EXÉCUTER next session] migration 012 (promote_avocat_on_confirm) écrite + commitée (501beab) mais PAS exécutée sur Supabase.** Exécuter (Ctrl+A → Run) puis lancer promote_avocat_on_confirm.test.sql (compte test 22349d59, reset_trigger_test_account.sql en filet si interruption). Reportée volontairement : touche auth.users (table système), à faire à tête reposée, pas en fin de session longue.
- [ ] **[Phase 1 — BLOQUANT avant promotion] auth_error=1 au clic du lien de confirmation email — CAUSE CONFIRMÉE par les logs :** liens de confirmation expirés (error_code=otp_expired, liens ouverts >1h après signup) ET rate limit email Supabase 2/h (empêche de recevoir de nouveaux liens rapidement). Ce n'est PAS un bug de code (le callback est correct). Solution racine = Resend SMTP (lève la limite + permet des liens exploitables à temps). Le trigger 012 rend d'ailleurs la promotion indépendante de auth_error=1 (déclenchée par la confirmation en base, pas par le callback).
...
### Session 012 (suite) — Trigger promotion 012 écrit + CODEMAP rattrapé + audit
Date: 2026-06-29 · Phase: 1 (Avocat Profile & Cabinet)
Fait: trigger 012 promote_avocat_on_email_confirm (AFTER UPDATE email_confirmed_at, défensif) + test simulé + reset script — commit 501beab, NON EXÉCUTÉ EN BASE. CODEMAP rattrapé migrations 006→012 + composants (commit d31f8c2). Audit projet complet.
Décisions: promotion par trigger sur email_confirmed_at = standard entreprise max (événement base, indépendant du callback/auth_error) · 012 NON exécuté en fin de session (touche auth.users → session dédiée à tête reposée) · auth_error=1 = liens expirés + SMTP 2/h, pas un bug code → Resend.
Bugs/état: 012 à exécuter+tester (1re tâche next) · auth_error=1 cause confirmée (otp_expired/SMTP) · Resend SMTP à configurer.
Commits: 501beab (012+test+reset), d31f8c2 (CODEMAP) · Prochaine session: exécuter+tester 012, puis Resend SMTP + fix auth_error=1.
